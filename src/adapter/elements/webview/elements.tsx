...
const Element: FunctionComponent<{ node: Cdp.DOM.Node }> = ({ node }) => {
    ...
    switch (node.nodeType) {
      ...
      case NodeType.ELEMENT_NODE:
        ...
        if (pseudoTypeElement) {
          ...
          // TODO
          // const pseudoElement = (
          //   <span class="webkit-html-pseudo-element">{'::' + pseudoTypeElement}</span>
          // );
  ...
  //     classes.push('close');
  //   }
  //   // const tagElement = <span class={classes.join(' ')}>{'<'}</span>;

  //   // const tagNameElement = (
  //   //   <span class={isClosingTag ? 'webkit-html-close-tag-name' : 'webkit-html-tag-name'}>
    ...
    <>
      <li>
        <div>
          ...
          <span class="highlight">{DealWithNodeTypes(node)}</span>
        </div>
      </li>
        ...
        <div class="elements-disclosure" tab-index={-1}>
          <ol class="elements-tree-outline source-code" tab-index={-1}>
            {!root || !root.children || root.children.length == 0
              ? ''
              : root.children.map(e => <Element node={e} />)}
          </ol>
        </div>
      </div>
    </div>
